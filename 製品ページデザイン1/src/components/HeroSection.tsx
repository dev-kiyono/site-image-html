...
export function HeroSection({ content }: HeroSectionProps) {
  ...
  return (
    <section className="relative bg-[#002040] text-white overflow-hidden">
      {/* 背景画像 */}
      <div className="absolute inset-0">
        {(content?.backgroundImage !== undefined ? content.backgroundImage : heroImage) && (
          <img
            src={content?.backgroundImage !== undefined ? content.backgroundImage : heroImage}
            alt="Xsens モーションキャプチャスーツ"
            className="w-full h-full object-cover opacity-80"
          />
        )}
        <div className="absolute inset-0 bg-gradient-to-r from-[#002040]/95 via-[#002040]/50 to-transparent" />
        <div className="absolute inset-0 bg-gradient-to-t from-[#002040]/60 via-transparent to-transparent" />
      </div>

      <div className="relative container mx-auto px-4 py-12 md:py-16 min-h-[65vh] flex items-center">
        <div className="max-w-xl ml-0 lg:ml-4">{/* テキスト幅をさらに狭めて画像スペース確保 */}
          {/* H1 */}
          <div className="mb-6">
            <div className="mb-6 h-12 md:h-16 w-32 md:w-40 relative">
              <NoImage />
            </div>
