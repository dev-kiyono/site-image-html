    ...
    <section className="py-10 md:py-12 bg-white">
      <div className="container mx-auto px-4">
        ...
        <div className="max-w-3xl mx-auto text-center mb-10">
          ...
          <h2 className="text-3xl md:text-4xl mb-4 text-[#1A1A1A]">Xsens MVNとは？</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-6xl mx-auto mb-12">
          {benefits.map((benefit, index) => (
            <div key={index} className="text-center border-2 border-[#f4f6f8] rounded-lg hover:border-[#CCFF00] transition-all hover:shadow-lg overflow-hidden">
              ...
              <div className="p-6">
                {/* タイトル */}
                <h3 className="text-xl mb-3 text-[#1A1A1A]">{benefit.title}</h3>

                {/* 説明 */}
                <p className="text-[#1A1A1A]">{benefit.description}</p>
              </div>
            </div>
          ))}
        </div>
