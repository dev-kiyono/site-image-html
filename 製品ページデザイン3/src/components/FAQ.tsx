    ...
    <section className="py-10 md:py-12 bg-[#002040]/30">
      <div className="container mx-auto px-4">
        <div className="max-w-3xl mx-auto text-center mb-10">
          <h2 className="text-white mb-4">よくある質問</h2>
        ...
        <div className="max-w-4xl mx-auto space-y-4">
          {data.faqs.map((faq, index) => (
            <div
              key={index}
              className="bg-[#002040]/50 border-2 border-[#CCFF00]/20 rounded-lg overflow-hidden hover:border-[#CCFF00] transition-all"
            >
              {/* 質問部分 */}
              <button
                onClick={() => toggleItem(index)}
                className="w-full px-6 py-5 flex items-start justify-between hover:bg-[#002040]/70 transition-colors text-left"
              >
                <div className="flex items-start gap-4 flex-1">
                  <div className="flex-shrink-0 w-8 h-8 bg-[#CCFF00] text-[#002040] rounded-full flex items-center justify-center text-sm mt-0.5">
                    Q
                  </div>
                  <span className="text-lg pr-4 text-white">{faq.question}</span>
                </div>
